/**
 * 节点测活 + 真实落地国家重命名 (Surge/Loon 版 - 支持查询失败加 -败)
 * 
 * 修改点：
 * 1. 测活成功 + GeoIP 查询成功 → 重命名为 国家-序号（如 US-01）
 * 2. 任何查询失败（GeoIP失败、WireGuard、无法产生 node 等）→ 保留原节点名 + 加 "-
 */
...
async function operator(proxies = [], targetPlatform, env) {
  ...
  for (let i = 0; i < validProxies.length; i++) {
    ...
    const originalName = proxy.name  // 保存原始名字

    // 如果有真实国家（非XX）→ 用国家-序号
    if (proxy._country && proxy._country !== 'XX') {
      let country = proxy._country
      ...
      proxy.name = `${country}-${seq}`
    } else {
      // 查询失败（GeoIP失败、WireGuard等）→ 原名 + -败
      proxy.name = `${originalName}-败`
    }
    delete proxy._country
  }
  // ====================== 重命名结束 ======================
  ...
  async function check(proxy) {
    ...
    try {
      // WireGuard 直接视为查询失败，走 -败 逻辑
      if (proxy.type === 'wireguard') {
        $.info(`[${proxy.name}] WireGuard 节点跳过测活`)
        validProxies.push({
          ...proxy,
          _country: 'XX'   // 标记为失败
        })
      ...
      if (node) {
        ...
        if (validStatus.test(status)) {
          ...
          try {
            ...
            $.error(`[${proxy.name}] GeoIP 查询失败 → 将使用原名-败`)
          }
          validProxies.push({
            ...proxy,
            _country: country,
