 ...
 * 实时油价监控小组件-本地版
 * 使用方法：
 * 1. 将此脚本完整保存为 Oil_Widget.JS
 * 2. 可选配置变量（在 widget 配置中设置 env）：
 *    - region：指定地区 slug（如 "guangdong/guangzhou" 或 "beijing"），留空则自动根据 IP 获取当地城市
 *    - SHOW_TREND：true / false，是否显示今日调价趋势，默认 true
 * 3. 自动定位基于 myip.ipip.net，支持中国主要城市
 * 4. 数据来源于 http://m.qiyoujiage.com
 *
  * 1️⃣ 环境变量配置
 * 
 * 在 Egern 小组件配置中添加：
  ...
  if (!regionParam || regionParam.trim() === "") {
    try {
      ...
      if (geoBody?.ret === "ok" && geoBody.data?.location && geoBody.data.location.length >= 3) {
        ...
        if (locArr[0] === "中国") {
          const cityCn = (locArr[2] || locArr[1] || "").replace(/市$|省$|自治区$|特别行政区$/, "");
          ...
          regionParam = map[cityCn] || "hainan/haikou";
        }
      }
    } catch (e) {}
    ...
    if (priceList.length >= 3) {
      ...
      if (SHOW_TREND) {
        ...
        if (trendMatch && trendMatch.length >= 3) {
          ...
          const trend = (valuePart.includes('下调') || valuePart.includes('下跌')) ? 'Down' : 'Up';
          let amount = "";
