...
async function startTasks() {
                ...
                setTimeout(() => {
                    ...
                    fetchApi('videoBonus').then(res => {
                        try {
                            ...
                            if (d.retcode === 0) {
                                msgs.push(`🎬 视频${i}：+${(d.result != null ? d.result.bonus : null) || '?'} Coins`);
                                resolve(next());
                            } else {
    ...
    await fetchApi('queryBalanceAndBonus')
        ...
        .then(res => {
            try {
                const d = JSON.parse(res.body);
                if (d.retcode === 0) msgs.push(`✅ 签到：${((d.result != null ? d.result.bonusHint : null) || d.retmsg || '').replace(/\n/g, ' ')}`);
