...
@modify 奶思做了修改，理论支持 Qx,Loon,Surge,Egern,ShadowRocket,青龙；linsar修复链式调用
...
async function startTasks() {
    ...
    await fetchApi('queryBalanceAndBonus')
        ...
        .then(async res => {
            ...
            if (!TG_NOTIFY_ONLY_FAIL) await sendTgNotify($.name + ' 🎉 任务完成', msgs.join('\n'));
        })
        .catch(async err => {
            if (!isNode) {
            ...
            await sendTgNotify($.name + ' ❌ 任务失败', msgs.join('\n') + '\n' + (err.error || String(err)));
        });
}
...
try {
    if (typeof $argument === 'string' && $argument) {
        ...
        TG_NOTIFY_ONLY_FAIL = arg.TG_NOTIFY_ONLY_FAIL === true || arg.TG_NOTIFY_ONLY_FAIL === 'true';
    }
} catch (e) {
    console.log('TG参数解析失败: ' + e);
}

async function sendTgNotify(title, message) {
