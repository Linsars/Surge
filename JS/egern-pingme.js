/*
 * PingMe 签到+视频奖励 执行脚本（Egern 专用）
 * 已修复 await + 支持 argument 传入参数
 */

const $ = new Env('PingMe签到');

const arg = $argument ? JSON.parse($argument) : {};
const raw = arg.PINGME_CAPTURE || $.getdata('pingme_capture_v3');
const NOTIFY_ONLY_FAIL = arg.NOTIFY_ONLY_FAIL || false;
...
if (!raw) {
    $.msg('❌ PingMe签到', '未找到参数', '请打开抓取开关并进入PingMe App获取参数');
    $.done();
}
...
async function startTasks() {
    console.log("开始运行PingMe签到");

    // 查询余额
    let res = await fetchApi('queryBalanceAndBonus');
    handleResponse(res, '查询余额', d => `💰 当前余额：${d.result.balance} Coins`);

    // 执行签到
    res = await fetchApi('checkIn');
    handleResponse(res, '签到', d => `✅ 签到成功：${(d.result?.bonusHint || d.retmsg || '获得奖励').replace(/\n/g, ' ')}`);

    // 刷5个视频
    for (let i = 1; i <= MAX_VIDEO; i++) {
        await new Promise(r => setTimeout(r, i === 1 ? 1500 : VIDEO_DELAY));
        res = await fetchApi('videoBonus');
        handleResponse(res, `视频${i}`, d => d.retcode === 0 ? `🎬 视频${i}：+${d.result?.bonus || '?'} Coins` : `⏸ 视频${i}：${d.retmsg}`);
    }

    // 最终余额
    res = await fetchApi('queryBalanceAndBonus');
    handleResponse(res, '最终余额', d => `💰 最新余额：${d.result.balance} Coins`);

    const result = msgs.join('\n');
    const isFail = result.includes('❌') || result.includes('⚠️');

    if (!NOTIFY_ONLY_FAIL || isFail) {
        $.msg('🎉 PingMe签到完成', '', result, { 'media-url': 'https://raw.githubusercontent.com/fmz200/wool_scripts/main/icons/apps/PingMe.png' });
    } else {
        console.log('仅失败通知模式，本次成功，跳过推送');
    }
}

function handleResponse(res, title, successMsg) {
    try {
        const d = JSON.parse(res.body);
        if (d.retcode === 0) {
            msgs.push(successMsg(d));
        } else {
            msgs.push(`⚠️ ${title}：${d.retmsg || '未知错误'}`);
        }
    } catch (e) {
        msgs.push(`❌ ${title}：解析失败`);
    }
}
...
// 执行
startTasks().catch(err => {
    console.log('签到异常：' + err);
    $.msg('❌ PingMe签到失败', '', err.message || String(err));
}).finally(() => $.done());
